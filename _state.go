...
func (ls *LState) NewLightUserData( ud interface{} ) *LightUserData {
	return &LightUserData{ value: ud }
}

func (ls *LState) NewUserDataByInterface(v interface{} , name string) *LUserData { //name: metatable name
	ud := ls.NewUserData()
	ud.Value = v
	mt := ls.GetTypeMetatable(name)
	ls.SetMetatable(ud , mt)
...
func (ls *LState) ToLightUserData(n int) *LightUserData {
	if lv, ok := ls.Get(n).(*LightUserData); ok {
		return lv
	}
	return nil
}
