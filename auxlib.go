...
func (ls *LState) CheckLightUserData(n int) *LightUserData {
	v := ls.Get(n)
	if lv, ok := v.(*LightUserData); ok {
		return lv
	}
	ls.TypeError(n, LTLightUserData)
	return nil
}
...
func (ls *LState) CheckSocket(n int) string {
    ...
    if !CheckSocket( v ) {
        ls.RaiseError("expected socket err: %s" , v)
        return ""
    }
    return v
...
func (ls *LState) CheckSockets(n int) string {
    ...
    for _ , item := range arr {
        if !CheckSocket( item ) {
            ls.RaiseError("expected socket err: %s" , item)
            return ""
        }
    }
...
func (ls *LState) OptLightUserData(n int, d *LightUserData) *LightUserData {
	v := ls.Get(n)
	if v == LNil {
		return d
	}
